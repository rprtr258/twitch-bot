...
func (cmd JoinCmd) Run(ctx context.Context, s *services.Services, perms []string, message twitch.PrivateMessage) (string, error) {
...
func (cmd LeaveCmd) Run(ctx context.Context, s *services.Services, perms []string, message twitch.PrivateMessage) (string, error) {
	words := strings.Split(message.Message, " ")
	if len(words) != 2 {
		return fmt.Sprintf("Usage: %s <channel>", cmd.Command()), nil
	}
